...
def call_gemini(prompt: str, system_prompt: str = None, temperature: float = 0.3) -> str:
    """Call the OpenRouter-backed Gemini wrapper with optional retries.

    This keeps the same signature used in this module but delegates to
    `services.gemini_client.call_gemini` which proxies to OpenRouter.
    """
    from .job_search import call_with_retry

    def _call():
        if system_prompt:
            # gemini_client.call_gemini accepts a `system` parameter
            return _openrouter_call(prompt, system=system_prompt, temperature=temperature)
        return _openrouter_call(prompt, temperature=temperature)

    # Use existing retry wrapper for rate-limit handling
